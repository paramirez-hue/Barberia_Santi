...
import { Settings, Calendar, Clock, Lock, Trash2, Camera, LogOut, LayoutDashboard, ListChecks, Save, Plus, Edit3, X } from 'lucide-react';
...
const AdminView: React.FC<AdminViewProps> = ({ config, setConfig, appointments, services, onSaveService, onDeleteService, onDeleteAppointment, onExit, showNotification }) => {
  ...
  const daysOfWeek = [
    ...
    { id: 3, label: 'Mie' }, { id: 4, label: 'Jue' }, { id: 5, label: 'Vie' }, { id: 6, label: 'Sab' },
  ];
    ...
    <div className="p-6 max-w-6xl mx-auto space-y-8 animate-in fade-in duration-500">
        ...
        {activeTab === 'dashboard' && (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div className="bg-zinc-900 border border-white/5 p-8 rounded-3xl">
              ...
              <p className="text-xl font-cinzel font-bold text-[#E2E8F0]">{todayAppointments[0]?.time || '--:--'}</p>
            </div>
          </div>
        )}

        {activeTab === 'appointments' && (
          <div className="bg-zinc-900 border border-white/5 rounded-3xl overflow-hidden shadow-2xl overflow-x-auto">
            <table className="w-full text-left min-w-[600px]">
              ...
              <tbody className="divide-y divide-white/5">
                {appointments.length === 0 ? (
                  <tr><td colSpan={4} className="px-6 py-10 text-center text-gray-500 font-cinzel italic">No hay registros</td></tr>
                ) : (
        ...
        {activeTab === 'services' && (
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {services.map(s => (
                <div key={s.id} className="bg-zinc-900 border border-white/5 p-6 rounded-3xl flex flex-col justify-between hover:border-[#E2E8F0]/30 transition-all group">
                  <div>
                  ...
                  <div className="flex justify-between items-center pt-4 border-t border-white/5">
                    <span className="text-[#E2E8F0] font-bold text-xl">${s.price.toFixed(2)}</span>
      ...
      {isServiceModalOpen && (
        <div className="fixed inset-0 bg-black/90 z-[60] flex items-center justify-center p-6 backdrop-blur-md">
          <div className="bg-zinc-900 border border-white/10 w-full max-w-xl rounded-3xl p-8 animate-in zoom-in-95 duration-300">
            ...
            <form onSubmit={handleServiceSubmit} className="space-y-5">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div className="space-y-2">
                  ...
                  <input required type="text" value={serviceFormData.name} onChange={e => setServiceFormData({...serviceFormData, name: e.target.value})} className="w-full bg-zinc-800 border border-white/5 rounded-xl p-4 text-white outline-none focus:border-[#E2E8F0]" placeholder="Ej: Degradado" />
                </div>
                <div className="space-y-2">
                  <label className="text-[10px] uppercase font-bold tracking-widest text-gray-500">Precio ($)</label>
                  <input required type="number" step="0.01" value={serviceFormData.price} onChange={e => setServiceFormData({...serviceFormData, price: e.target.value})} className="w-full bg-zinc-800 border border-white/5 rounded-xl p-4 text-white outline-none focus:border-[#E2E8F0]" placeholder="0.00" />
                </div>
                <div className="space-y-2">
                  <label className="text-[10px] uppercase font-bold tracking-widest text-gray-500">Duración (min)</label>
                  <input required type="number" value={serviceFormData.durationMinutes} onChange={e => setServiceFormData({...serviceFormData, durationMinutes: e.target.value})} className="w-full bg-zinc-800 border border-white/5 rounded-xl p-4 text-white outline-none focus:border-[#E2E8F0]" />
                </div>
                <div className="space-y-2">
                  <label className="text-[10px] uppercase font-bold tracking-widest text-gray-500">Categoría</label>
                  <select value={serviceFormData.category} onChange={e => setServiceFormData({...serviceFormData, category: e.target.value})} className="w-full bg-zinc-800 border border-white/5 rounded-xl p-4 text-white outline-none appearance-none">
              ...
              <div className="space-y-2">
                <label className="text-[10px] uppercase font-bold tracking-widest text-gray-500">Descripción Corta</label>
                <textarea rows={3} value={serviceFormData.description} onChange={e => setServiceFormData({...serviceFormData, description: e.target.value})} className="w-full bg-zinc-800 border border-white/5 rounded-xl p-4 text-white outline-none focus:border-[#E2E8F0]" placeholder="Breve detalle del servicio..." />
              </div>
              ...
              <div className="pt-4 flex space-x-4">
                ...
                <button type="submit" className="flex-[2] gold-gradient text-black font-bold uppercase py-4 rounded-xl text-xs tracking-widest shadow-xl">Guardar Servicio</button>
              </div>
            </form>
          </div>
        </div>
